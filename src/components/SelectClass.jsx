      ...
      <div className="flex flex-col items-center gap-8">
        ...
        <div className="flex flex-wrap justify-center w-full gap-4">
          {classTypes.map((chosenClass) => (
            <button
              key={chosenClass.name}
              onClick={() => {
                handleClassClick(chosenClass.name);
              }}
              className=""
            >
              <img
                className={`w-24 h-24 duration-500 border-4 ${
                  classType === chosenClass.name
                    ? "border-green-500"
                    : "border-orange-900 rounded-lg hover:border"
